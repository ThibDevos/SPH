...
template <size_t D>
void radix_sort_indices(std::vector<uint64_t>& codes, std::vector<int>& indices, Particles<D>& particles)
{
  size_t n = codes.size();
  if (n == 0) return;
      ...
      for (size_t i = 0; i < n; i++)
        {
          ...
          temp_particles.density(pos) = particles.density(i);
          temp_particles.pressure(pos) = particles.pressure(i);
          temp_particles.leaf(pos) = particles.leaf(i);
          // no need to sort neighbours as they are computed afterwards
          for(int d=0; d<D; ++d)
          {
            temp_particles.position(pos,d) = particles.position(i,d);
            temp_particles.velocity(pos,d) = particles.velocity(i,d);
            temp_particles.force(pos,d) = particles.force(i,d);
          }
        }
...
{
public:
  Morton_SoA(Particles<D> &particles, double h)
  {
      int nb_part = particles.nb_particles;
  ...
  //XXX also compute l
  void float_coord_to_int(Particles<D> & particles, std::vector<std::array<uint32_t,D>>& int_coords, double h )
  {
    ...
    for(int i=0; i<particles.nb_particles; ++i)
    {
      for(int d=0; d<D; ++d)
      {
        double pos = particles.position(i,d);
    ...
    for(int i=0; i<particles.nb_particles; ++i)
    {
      for (int d = 0; d < D; ++d)
      {
        int_coords[i][d] = static_cast<uint32_t>(std::clamp((particles.position(i,d) - min[d])/Max, 0., 1.) * max_coord);
      }
    }
  ...
  /*
    ...
    */
    void build_leaves(Particles<D> & particles)
    {
      for (int d = 0; d < D; ++d)
      {
        grid_limits[d] = 0;
    ...
    void compute_neighbours(Particles<D> & particles)
    {
      ...
      for (auto it = leaves.begin(); it != leaves.end(); ++it)
      {
        ...
        std::vector<int> neighbouring_parts;
        neighbouring_parts.reserve(it->vertex_indices.size() * pow3); //estimating the number of particles 
        uint32_t nx[D];
        for(int c=0; c<pow3; ++c) //3^D is the number of leaves in the cluster
        {
          ...
          if (found_it != leaves.end() && found_it->Morton_code == n_code)
          {
            ...
            {
              neighbouring_parts.push_back(found_it->vertex_indices[i]);
            }
          }
        }
        ...
        {
          int current_part = current_leaf.vertex_indices[i];
          particles.neighbour(current_part) = neighbouring_parts;
          for(int j=0; j<current_leaf.vertex_indices.size(); ++j)
          {
            particles.neighbour(current_part).push_back(current_leaf.vertex_indices[j]);
          }
